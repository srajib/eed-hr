...
{
	if(com_id)
		window.open('<?php echo $this->base; ?>/admin/employees/empreport/'+com_id,'','scrollbars=yes,menubar=yes,height=925,width=800,resizable=no,toolbar=no,location=no,addressbar=no,status=no,dialog=yes');
...
<p>&nbsp;</p>
 <fieldset><legend>Employee Info by Employee</legend>
<p>  
	<div class="input"> 
	   <?php echo $this->Form->label('Employee');  ?>   
	    <?php echo $this->Form->select('employee_id', $designations,array('empty'=>false));?>   
  ...
  <div  style="float:left;margin:10px;">
    <input type="submit" name="report" value="Staff Report" onClick="Report(EmployeeEmployeeId.value);">
  </div>
